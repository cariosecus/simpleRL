...
from render_functions import clear_all, render_all, get_map_offset, get_console_offset
...
# main game loop
	while True:
		...
		if render_update:
			...
			tcod.console_flush()
			render_update = False
		if msg_up and message_log.bottom < message_log.length - message_log.height:
				message_log.scroll(1)
				render_update = True

		if msg_down and message_log.bottom > 0:
				message_log.scroll(-1)
				render_update = True
